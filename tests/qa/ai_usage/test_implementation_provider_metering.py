...
def context(workspace: Path, org="org-a"):
    return EngineeringImplementationContext(execution_id="exec-1",workspace=workspace,organization_id=org,user_id="user-a",project_id="project-a",session_id="session-a",task="task",analysis=BoundedProjectAnalysis(),plan=PLAN,step=STEP)
...
def test_ai_backed_provider_is_metered_once_with_trusted_context(tmp_path: Path):
    ...
    assert provider.changes_for(context(tmp_path))[0].relative_path == "app.py"
...
def test_already_metered_provider_is_not_double_counted(tmp_path: Path):
    repository=InMemoryAIUsageRepository()
    MeteredEngineeringImplementationProvider(AIProvider(already_metered=True),AIUsageService(repository)).changes_for(context(tmp_path))
    assert repository.query("org-a") == ()

def test_failed_ai_provider_records_failed_once_and_reraises(tmp_path: Path):
    ...
    with pytest.raises(RuntimeError,match="provider failed"): provider.changes_for(context(tmp_path))
