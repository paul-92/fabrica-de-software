...
def test_default_app_publishes_project_engineering_routes(tmp_path: Path) -> None:
    app = create_default_app(
        ApplicationSettings(hosted_root=tmp_path / "hosted")
    )
...
def test_project_http_round_trip(tmp_path: Path) -> None:
    ...
    client = TestClient(
        create_default_app(
            ApplicationSettings(hosted_root=hosted_root)
        )
    )
    ...
    assert project["workspace_kind"] == "hosted"
...
def test_project_http_validation_and_not_found(
    tmp_path: Path,
) -> None:
    client = TestClient(
        create_default_app(
...
def test_session_http_round_trip_and_strict_contract(
    tmp_path: Path,
) -> None:
    client = TestClient(
        create_default_app(
...
def test_workspace_listing_read_and_safe_errors(
    tmp_path: Path,
) -> None:
    hosted_root = tmp_path / "hosted"
    ...
    base = (
        f"/api/v1/projects/{project['project_id']}/workspace"
    )
    ...
    content = client.get(
        f"{base}/file",
        params={"path": "src/sample.py"},
    ).json()

    assert content["content"].splitlines() == [
        "print('olÃ¡')"
    ]
    assert content["path"] == "src/sample.py"
    ...
    assert client.get(
        f"{base}/file",
        params={"path": "binary"},
    ).json()["error"]["code"] == "WORKSPACE_BINARY_FILE"

    assert (
        client.get(
            "/api/v1/projects/missing/workspace"
        ).status_code
        == 404
    )