...
from asep.ai_runtime import AIRuntimeExecutionMode, AIRuntimeIdentity, AIRuntimeResult, InMemoryAIRuntimeRegistry
from asep.application import ProjectAIRuntimeExecutionRequest, ProjectAIRuntimeExecutionService, ProjectService, ProjectSessionService
from asep.application.project_engineering_planning import DeterministicEngineeringTaskDecomposer, ProjectEngineeringPlanningService
from asep.project_analysis import ProjectAnalyzer
from asep.projects import InMemoryProjectExecutionRepository, InMemoryProjectRepository, InMemoryProjectSessionRepository, ProjectEngineeringStepResult, WorkspaceProject
...
class Executor:
    ...
    def execute_supported_plan(self, execution, plan, workspace, analysis):
        ...
        return tuple(ProjectEngineeringStepResult(
...
def make_service(tmp_path: Path):
    ...
    projects.save(WorkspaceProject(project_id="p-1", name="Project", workspace_path=tmp_path, created_at=datetime.now(UTC), updated_at=datetime.now(UTC)))
    executions = InMemoryProjectExecutionRepository()
    project_service = ProjectService(projects)
    sessions = ProjectSessionService(project_service, InMemoryProjectSessionRepository(), executions, id_generator=lambda: "s-1")
    ...
    service = ProjectAIRuntimeExecutionService(
        project_service, registry, sessions, executions,
        engineering_planning=ProjectEngineeringPlanningService(ProjectAnalyzer(), DeterministicEngineeringTaskDecomposer()),
        internal_execution=executor,
        defer_completion=True,
        id_generator=lambda: "e-prepare",
    )
    request = ProjectAIRuntimeExecutionRequest(project_id="p-1", session_id="s-1", runtime_id="codex", instruction="Add endpoint", execution_mode=AIRuntimeExecutionMode.WORKSPACE_WRITE)
...
def test_cancel_is_terminal_and_does_not_mutate(tmp_path: Path):
    service, request, executions, runtime, executor = make_service(tmp_path)
    prepared = service.prepare(request)
    before = (tmp_path / "app.py").read_bytes()
    cancelled = service.cancel_prepared(prepared.execution_id, request)
    assert (tmp_path / "app.py").read_bytes() == before
    assert cancelled.status.value == "failed"
    assert cancelled.error_code == "PREPARATION_CANCELLED"
    assert cancelled.completed_at is not None
    assert executions.get(prepared.execution_id) == cancelled
    assert runtime.calls == executor.calls == 0
    with pytest.raises(ValueError, match="not available"):
        service.execute_prepared(prepared.execution_id, request)
