...
function ContextUsage({ count, truncated, charCount, omittedCount }: { count: number; truncated: boolean; charCount: number; omittedCount: number }) {
  ...
  return <p>Using context from {count} previous {count === 1 ? "execution" : "executions"}. Context size: {formatContextChars(charCount)}.{omittedCount > 0 ? ` ${omittedCount} older ${omittedCount === 1 ? "execution" : "executions"} omitted.` : ""}{truncated ? " Recent context was compacted." : ""}</p>;
}
