...
export function ProjectRuntimePanel({ projectId, projectName, workspacePath, service }: Props) {
  ...
  useEffect(() => {
    ...
    api.listSessions(projectId).then((items) => { if (active) { setSessions(items); setSessionsLoadError(false); setHistory(null); setSelectedExecution(null); setSelectedSession(items[0] ?? null); } }, () => { if (active) { setSessions([]); setSessionsLoadError(true); } });
    return () => { active = false; };
  ...
  async function createSession(event: FormEvent) {
    ...
    try {
      ...
      setHistory(null); setSelectedExecution(null); setSelectedSession(created); setSessionTitle("");
  ...
  return <div className="page-stack">
    ...
    {selectedSession ? <Card title={selectedSession.title} eyebrow="AI Runtime">
      ...
      {result ? <div className="runtime-result"><h3>Codex result</h3><pre>{result.output}</pre><p>Execution mode: {result.execution_mode}</p><h4>Workspace changes</h4>{result.changes.length === 0 ? <p>No workspace changes detected.</p> : <ChangeList changes={result.changes} />}</div> : null}
    </Card> : null}
